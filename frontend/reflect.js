...
class Pair extends HeapObject {
    ...
    repr() {
        ...
        if (cdr_repr == '()') return `(${car_repr})`
        if (cdr_repr.charAt(0) == '(') return `(${car_repr} ${cdr_repr.substring(1)}`
...
class MutableString extends HeapObject {
    ...
    repr() {
        return string_repr(this.reflector.string_value(this))
    }
}
...
class Sym extends HeapObject {
    ...
    repr() {
        return this.reflector.symbol_name(this)
    }
}
...
class Keyword extends HeapObject {
    ...
    repr() {
        return `#:${this.reflector.keyword_name(this)}`
    }
}

class Variable extends HeapObject {
    toString() {
        return '#<variable>'
    }
}
class AtomicBox extends HeapObject {
    toString() {
        return '#<atomic-box>'
    }
}
class HashTable extends HeapObject {
    toString() {
        return '#<hash-table>'
    }
}
class WeakTable extends HeapObject {
    toString() {
        return '#<weak-table>'
    }
}
class Fluid extends HeapObject {
    toString() {
        return '#<fluid>'
    }
}
class DynamicState extends HeapObject {
    toString() {
        return '#<dynamic-state>'
    }
}
class Syntax extends HeapObject {
    toString() {
        return '#<syntax>'
    }
}
class Port extends HeapObject {
    toString() {
        return '#<port>'
    }
}
class Struct extends HeapObject {
    toString() {
        return '#<struct>'
    }
}
...
class IterableWeakSet {
    ...
    #kill(i) {
        ...
        if (i < this.#array.length) this.#array[i] = tail
    }
    #get(i) {
    ...
    add(x) {
        ...
        if (this.#array.length % 32 == 0) this.#cleanup()
        this.#set.add(x)
    ...
    *[Symbol.iterator]() {
        for (let i = 0, x; (x = this.#get(i)); i++) yield x
    }
}
...
class Scheme {
    ...
    static async reflect(abi, { reflect_wasm_dir = '.' }) {
        ...
        let rt = {
            quit(status) {
                throw new SchemeQuitError(status)
            },
            die(tag, data) {
                throw new SchemeTrapError(tag, data)
            },
            wtf8_to_string(wtf8) {
                return wtf8_to_string(wtf8)
            },
            string_to_wtf8(str) {
                return string_to_wtf8(str)
            }
        }
        let { module, instance } = await instantiate_streaming(reflect_wasm, { abi, debug, rt })
    ...
    #to_scm(js) {
        ...
        if (typeof js == 'number') {
            ...
            if (
                BigInt(api.scm_most_negative_fixnum()) <= js &&
            ...
            if (js instanceof Fraction)
                return api.scm_from_fraction(this.#to_scm(js.num), this.#to_scm(js.denom))
            if (js instanceof Complex) return api.scm_from_complex(js.real, js.imag)
    ...
    #to_js(scm) {
        ...
        let handlers = {
            ...
            complex: () => new Complex(api.complex_real(scm), api.complex_imag(scm)),
            fraction: () =>
                new Fraction(
                    this.#to_js(api.fraction_num(scm)),
    ...
    call(func, ...args) {
        ...
        for (let [idx, arg] of args.entries()) api.vector_set(argv, idx + 1, this.#to_scm(arg))
    ...
    car(x) {
        return this.#to_js(this.#instance.exports.car(x.obj))
    }
    cdr(x) {
        return this.#to_js(this.#instance.exports.cdr(x.obj))
    }

    vector_length(x) {
        return this.#instance.exports.vector_length(x.obj)
    }
    vector_ref(x, i) {
    ...
    string_value(x) {
        return this.#instance.exports.string_value(x.obj)
    }
    symbol_name(x) {
        return this.#instance.exports.symbol_name(x.obj)
    }
    keyword_name(x) {
        return this.#instance.exports.keyword_name(x.obj)
    }
}
...
class SchemeTrapError extends Error {
    ...
    // have a toString.
    toString() {
        return `SchemeTrap(${this.tag}, <data>)`
    }
}
...
class SchemeQuitError extends Error {
    ...
    toString() {
        return `SchemeQuit(status=${this.status})`
    }
}
...
let async_invoke =
    typeof queueMicrotask !== 'undefined' ? queueMicrotask : thunk => setTimeout(thunk, 0)
...
function wtf8_to_string(wtf8) {
    ...
    for (let cp = iter_next(iter); cp != -1; cp = iter_next(iter)) codepoints.push(cp)
...
function string_to_wtf8(str) {
    let { string_builder, builder_push_codepoint, finish_builder } = wtf8_helper.exports
    let builder = string_builder()
    for (let cp of str) builder_push_codepoint(builder, cp.codePointAt(0))
...
class SchemeModule {
    ...
    static #rt = {
        ...
        bignum_from_u64(n) {
            return n < 0n ? 0xffff_ffff_ffff_ffffn + (n + 1n) : n
        },
        bignum_is_i64(n) {
        ...
        weak_map_set(map, k, v) {
            return map.set(k, v)
        },
        weak_map_delete(map, k) {
            return map.delete(k)
        },
        ...
        make_regexp(pattern, flags) {
            return new RegExp(pattern, flags)
        },
        regexp_exec(re, str) {
            return re.exec(str)
        },
        regexp_match_string(m) {
            return m.input
        },
        regexp_match_start(m) {
            return m.index
        },
        regexp_match_end(m) {
            return m.index + m[0].length
        },
        regexp_match_count(m) {
            return m.length
        },
        ...
        stream_make_chunk(len) {
            return new Uint8Array(len)
        },
        stream_chunk_length(chunk) {
            return chunk.length
        },
        stream_chunk_ref(chunk, idx) {
            return chunk[idx]
        },
        stream_chunk_set(chunk, idx, val) {
            chunk[idx] = val
        },
        stream_get_reader(stream) {
            return stream.getReader()
        },
        stream_read(reader) {
            return reader.read()
        },
        stream_result_chunk(result) {
            return result.value
        },
        stream_result_done(result) {
            return result.done ? 1 : 0
        },
        stream_get_writer(stream) {
            return stream.getWriter()
        },
        stream_write(writer, chunk) {
            return writer.write(chunk)
        },
        stream_close_writer(writer) {
            return writer.close()
        }
    }
    ...
    static #code_origin(code) {
        if (SchemeModule.#code_origins.has(code)) return SchemeModule.#code_origins.get(code)
        for (let mod of SchemeModule.#all_modules) {
            for (let i = 0, x = null; (x = mod.instance_code(i)); i++) {
                let origin = [mod, i]
                if (!SchemeModule.#code_origins.has(x)) SchemeModule.#code_origins.set(x, origin)
                if (x === code) return origin
            }
        }
        return [null, 0]
    }
    ...
    static #code_name(code) {
        ...
        if (mod) return mod.instance_code_name(idx)
        return null
    }

    static #code_source(code) {
        let [mod, idx] = SchemeModule.#code_origin(code)
        if (mod) return mod.instance_code_source(idx)
        return [null, 0, 0]
    }
    ...
    constructor(instance) {
        ...
        if (typeof printErr === 'function') {
            ...
            let write_no_newline =
                typeof write === 'function' ? write : typeof putstr === 'function' ? putstr : print
            ...
            this.#io_handler = {
                ...
                stream_stdin() {
                    throw new Error('stream_stdin not implemented')
                },
                stream_stdout() {
                    throw new Error('stream_stderr not implemented')
                },
                stream_stderr() {
                    throw new Error('stream_stderr not implemented')
                }
        ...
        this.#debug_handler = {
            ...
            debug_str_scm(x, y) {
                console.log(`debug: ${x}: #<scm>`)
            }
        }
    }
    static async fetch_and_instantiate(path, { abi, reflect_wasm_dir = '.', user_imports = {} }) {
        await load_wtf8_helper_module(reflect_wasm_dir)
        let io = {
            write_stdout(str) {
                mod.#io_handler.write_stdout(str)
            },
            write_stderr(str) {
                mod.#io_handler.write_stderr(str)
            },
            read_stdin() {
                return mod.#io_handler.read_stdin()
            },
            file_exists(filename) {
                return mod.#io_handler.file_exists(filename)
            },
            open_input_file(filename) {
                return mod.#io_handler.open_input_file(filename)
            },
            open_output_file(filename) {
                return mod.#io_handler.open_output_file(filename)
            },
            close_file(handle) {
                mod.#io_handler.close_file(handle)
            },
            read_file(handle, length) {
                return mod.#io_handler.read_file(handle, length)
            },
            write_file(handle, length) {
                return mod.#io_handler.write_file(handle, length)
            },
            seek_file(handle, offset, whence) {
                return mod.#io_handler.seek_file(handle, offset, whence)
            },
            file_random_access(handle) {
                return mod.#io_handler.file_random_access(handle)
            },
            file_buffer_size(handle) {
                return mod.#io_handler.file_buffer_size(handle)
            },
            file_buffer_ref(handle, i) {
                return mod.#io_handler.file_buffer_ref(handle, i)
            },
            file_buffer_set(handle, i, x) {
                return mod.#io_handler.file_buffer_set(handle, i, x)
            },
            delete_file(filename) {
                mod.#io_handler.delete_file(filename)
            },
            stream_stdin() {
                return mod.#io_handler.stream_stdin()
            },
            stream_stdout() {
                return mod.#io_handler.stream_stdout()
            },
            stream_stderr() {
                return mod.#io_handler.stream_stderr()
            }
        }
        let debug = {
            debug_str(x) {
                mod.#debug_handler.debug_str(x)
            },
            debug_str_i32(x, y) {
                mod.#debug_handler.debug_str_i32(x, y)
            },
            debug_str_scm(x, y) {
                mod.#debug_handler.debug_str_scm(x, y)
            },
            code_name(code) {
                return SchemeModule.#code_name(code)
            },
            code_source(code) {
                return SchemeModule.#code_source(code)
            }
        }
        let ffi = {
    ...
    exports() {
        let ret = {}
        for (let [k, v] of Object.entries(this.all_exports())) {
            if (!k.startsWith('$')) ret[k] = v
        }
        return ret
    }
    get_export(name) {
        if (name in this.all_exports()) return this.all_exports()[name]
...
function repr(obj) {
    if (obj instanceof HeapObject) return obj.repr()
    if (typeof obj === 'boolean') return obj ? '#t' : '#f'
    if (typeof obj === 'number') return flonum_to_string(obj)
    if (typeof obj === 'string') return string_repr(obj)
    return obj + ''
}
